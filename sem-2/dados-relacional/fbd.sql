...
select Nome_cliente, nome_vendedor, descricao, Valor_unitario, quantidade, 
Valor_unitario*quantidade as total from pedido as p
left join cliente as cl on cl.Cod_cliente = p.cod_cliente
left join vendedor as v on v.cod_vendedor = v.cod_vendedor 
left join item_pedido as i on i.num_pedido = p.num_pedido
left join produto as pr on pr.cod_produto = i.cod_produto
